  ...
  class Birds
    ...
    def update(tags)
      ...
      while all_new
        ...
        empty = result.respond_to?(:empty) ? result.empty? : false
        all_new = !empty && results.all? { |item| add_tweet(item) && result << item } #!results.empty? && results.all? { | item | add_tweet(item) && result << item }
